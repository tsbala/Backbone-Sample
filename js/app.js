...
SearchTwitter.Tweet = Backbone.Model.extend({
    select: function () {
        ...
        SearchTwitter.vent.trigger('tweet:selected', this);
    }
});
...
SearchTwitter.Pagination = Backbone.Model.extend({
    defaults: { 
        nextPage: ''
    }
});

SearchTwitter.Pager = Backbone.View.extend({
    template: '#pagination',
    render: function () {
...
(function ($) {
    ...
    SearchTwitter.vent.bind('tweetsFound', function (data, searchTerm) {
        ...
        if (data.next_page) {
            var pagination = new SearchTwitter.Pagination({nextPage: data.next_page});
            var pager = new SearchTwitter.Pager({ model : pagination});
            pager.render();
            $(pager.el).appendTo($('div.container'));
        }
    });
